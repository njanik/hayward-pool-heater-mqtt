...
boolean mqttMsgReceivedCallBack(char* topic, byte* payload, unsigned int length) {
  ...
  if(currentMode == 255 || currentProgTemp == 0) {  //255 is a dummy value that i've used to init the mode
...
void loop() {
  ...
  if (!state) {
    if (highCpt) {
      ...
      if (highCpt >= 22 && highCpt <= 28) { //5ms = 25 * 200us
        // START TRAME
        cptBuffer = 0;
        buffer = 0;
        wordCounter = 0;

      } else if (highCpt >= 12 && highCpt <= 18) {  //3ms = 15 * 200us
        // BINARY O
        buffer = buffer << 1;
        cptBuffer++;

      } else if (highCpt >= 2 && highCpt <= 8) {  //1ms = 5 * 200us
        // BINARY 1
        buffer = buffer << 1;
        buffer = buffer | 1;
        cptBuffer++;
      ...
      if (cptBuffer == 8) {
        ...
        if (wordCounter < 20) { //overflow verification
          wordCounter++;
        }
        cptBuffer = 0;
        buffer = 0;
    ...
    highCpt++;
    if (highCpt > (50000 / 200) && wordCounter) {    //If HIGH more than 50ms, trame is hardly ended
